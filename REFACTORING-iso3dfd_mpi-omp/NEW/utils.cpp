...
// =============================================================
//
// Modified: Added InitializeLocal for MPI domain decomposition
//
...
/*
 * Host-Code
 * Function used for initialization (original, full grid)
 */
...
// =============================================================
// MPI: Local initialization for domain decomposition
// =============================================================
#ifdef USE_MPI
/*
 * Host-Code
 * Initializes local subdomain for a given MPI rank.
 *
 * Zeros out the local array (including halo) and sets velocity.
 * Places the seismic source at the global grid center if it
 * falls within this rank's computational domain.
 *
 * Parameters:
 *   n1, n2       - full grid dimensions (including X/Y halo)
 *   n3_alloc     - local Z dimension (including top/bottom halo)
 *   n3_local     - local computational Z cells (without halo)
 *   z_start_global - global Z start of this rank (in full-array coords)
 *   n3_global    - full Z dimension (including halo) of the global grid
 *
 * Local array layout in Z:
 *   [0 .. kHalfLength-1]                     -> bottom halo
 *   [kHalfLength .. kHalfLength+n3_local-1]  -> computational domain
 *   [kHalfLength+n3_local .. n3_alloc-1]     -> top halo
 *
 * Mapping: global_z = z_start_global + (local_z - kHalfLength)
 */
void InitializeLocal(float* ptr_prev, float* ptr_next, float* ptr_vel,
  ...
  // Zero out and set velocity for the entire local array (including halo)
  for (size_t i = 0; i < n3_alloc; i++) {
    for (size_t j = 0; j < n2; j++) {
      auto offset = i * dim2 + j * n1;
      for (size_t k = 0; k < n1; k++) {
        ptr_prev[offset + k] = 0.0f;
        ptr_next[offset + k] = 0.0f;
        ptr_vel[offset + k] = 2250000.0f * dt * dt;
      }
    }
  }

  // Place source at global grid center
  // Source position in global array: Z = n3_global/2, Y = n2/4, X = n1/4
  float val = 1.f;
  for (int s = 5; s >= 0; s--) {
    for (int i = (int)(n3_global / 2) - s; i < (int)(n3_global / 2) + s; i++) {
      // Map global Z index to local Z index
      int local_i = i - (int)z_start_global + kHalfLength;
      // Check if this global Z layer falls within our computational domain
      if (local_i >= (int)kHalfLength &&
          local_i < (int)(kHalfLength + n3_local)) {
        for (int j = (int)(n2 / 4) - s; j < (int)(n2 / 4) + s; j++) {
          auto offset = (size_t)local_i * dim2 + (size_t)j * n1;
          for (int k = (int)(n1 / 4) - s; k < (int)(n1 / 4) + s; k++) {
            ptr_prev[offset + (size_t)k] = val;
          }
        }
      }
    }
